const SHEET_URL = "https://docs.google.com/spreadsheets/d/e/2PACX-1vTt3orOXvvfiKTCtlAhWbsBe5HNriBWTrESKih-PMPnncLdx6xYrsVm65LscTdvuhnj_HZRyUiaZnfz/pub?gid=63771902&single=true&output=csv";
const START_HOUR = 6;
const END_HOUR = 24;
const FIXED_ROOMS = [
  "Ruang Rapat (Lt. 1)", "Hall Tengah (Lt. 1)", "Hall Selatan (Lt. 1)",
  "L1.101 (Profesi)", "L1.102 (Profesi)", "L1.103 (Profesi)", "L1.104 (Profesi)", "L1.105 (Profesi)",
  "L1.201 (Profesi)", "L1.202 (Profesi)", "L1.203 (Profesi)", "L1.204 (Profesi)", "L1.205 (Profesi)",
  "L0.201 (Gedung Lama)", "L0.205 / Smart Classroom (Gedung Lama)",
  "Ruang Pusat Studi (Lt. 2)", "Ruang Baca Sarjana (Lt. 2)", "Ruang Baca Kubikus Profesi/Mapro/Magister/Doktor (Lt. 2)",
  "Ruang Hybrid (Lt.2)", "Ruang Podcast Psikologi (Lt. 2)",
  "L0.301 (Gedung Lama)", "L0.302 (Gedung Lama)", "L0.303 (Gedung Lama)", "L0.304 (Gedung Lama)", "L0.305 (Gedung Lama)",
  "L0.305 (Pasca)", "L1.301 (Profesi)", "L1.302 (Profesi)", "L1.303 (Profesi)", "L1.304 (Profesi)", "L1.305 (Profesi)"
];

const BOOKING_COLORS = [
    { bg: "bg-blue-50", border: "border-blue-500", text: "text-blue-700", subtext: "text-blue-500", hover: "hover:shadow-md" },
    { bg: "bg-emerald-50", border: "border-emerald-500", text: "text-emerald-700", subtext: "text-emerald-600", hover: "hover:shadow-md" },
    { bg: "bg-amber-50", border: "border-amber-500", text: "text-amber-700", subtext: "text-amber-600", hover: "hover:shadow-md" },
    { bg: "bg-rose-50", border: "border-rose-500", text: "text-rose-700", subtext: "text-rose-600", hover: "hover:shadow-md" },
    { bg: "bg-purple-50", border: "border-purple-500", text: "text-purple-700", subtext: "text-purple-500", hover: "hover:shadow-md" },
    { bg: "bg-cyan-50", border: "border-cyan-500", text: "text-cyan-700", subtext: "text-cyan-600", hover: "hover:shadow-md" },
];
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    if(dateEl) {
        ...
        dateEl.addEventListener("changeDate", (e) => {
            currentFilterDate = e.detail && e.detail.date ? e.detail.date : new Date(dateEl.value);
            renderGrid();
        });
    }
    ...
    data.forEach((row) => {
        ...
        if (rawStart && rawEnd && roomRaw) {
            ...
            if (startObj && endObj) {
                ...
                const separateRooms = roomRaw.split(/,|\n| dan |&/i).map(r => r.trim()).filter(r => r.length > 0);
                separateRooms.forEach(singleRoomName => {
                    foundRooms.add(singleRoomName);
                    allBookings.push({
                        start: startObj, end: endObj, room: singleRoomName,
...
function renderGrid() {
    ...
    for (let h = START_HOUR; h < END_HOUR; h++) {
        ...
        displayRooms.forEach((room) => {
            ...
            if (booking) {
                ...
                empty.className = "w-full h-full rounded hover:bg-gray-50/50 transition-colors flex items-center justify-center opacity-0 hover:opacity-100 cursor-pointer";
    ...
    if (emptyState) {
        if (dailyBookingCount === 0 && displayRooms.length === 0) emptyState.classList.remove("hidden");
